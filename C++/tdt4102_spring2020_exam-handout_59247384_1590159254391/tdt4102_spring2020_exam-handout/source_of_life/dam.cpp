...
int Dam::water_level_delta() const
{
  ...
  return (abs(water_level - ideal_level));
...
Dam::WaterAlarm Dam::get_alarm() const
{
  ...
  if (water_level > water_level_max){
    return static_cast<WaterAlarm>(1);
  }
  else if ( water_level < water_level_min)
  {
    return static_cast<WaterAlarm>(0);
  }
  else
  {
...
void Dam::restore_water_level()
{
  ...
  // code that is already there and replace it with your own.
  if(water_level_delta()< inflow_capacity || water_level_delta()< outflow_capacity ){
    water_level = ideal_level;
  }
  /*
