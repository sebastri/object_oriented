...
Color::Color_type Plotter::get_color(const string dam)
{
  ...
  dam_colors[dam] = colors.next_color();
                         ...
                         Dam::WaterAlarm alarm)
{
  ...
  if (alarm)
  {
    Open_polyline* l = new Open_polyline;
    l->add(Point{calc_x(timestep), y_len + 10});
    l->set_color(get_color(dam));
    shapes.push_back(l);
...
void Plotter::plot_limit(const string dam, unsigned int limit)
{
  ...
  Open_polyline* horisontal = new Open_polyline;
  horisontal->set_color(get_color(dam));
  horisontal->add(Point{calc_x(0), calc_y(limit)});
  shapes.push_back(horisontal);
