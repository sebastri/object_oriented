...
public:
  ...
  ~Vector2d();
  double length();
  friend Vector2d operator*(Vector2d vec, double k);
};
Vector2d::~Vector2d()
{
}
...
vector<Vector2d> cleanTrack(vector<Vector2d>& track){
  Vector2d v(0,0);
  transform(track.begin(), track.end(), track.begin(), v);
  return track;
}

void trackStats(const vector<Vector2d>& track){
  double distance = vectorSum(track).length();
  double max = 0;
  for(auto p:track){
    if (p.length()/10>max)
    {
  ...
  Vector2d sluttvector = vectorSum(track);
  cout << "Length: " << setprecision(2) << distance << ", max-speed: " << max << " m/min, ended at " << sluttvector << endl; 
}
...
Vector2d operator*(Vector2d vec, double k){
