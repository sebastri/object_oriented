...
void addGuess(MastermindWindow& mwin,const string code,const int size, const char startLetter, const int round){
	...
	{
	mwin.vr.push_back(new Rectangle{Point{static_cast<int>(a+1)*padX,yPos},padX,padY});
...
void addFeedback(MastermindWindow &mwin, int correctPosition, int correctCharacter, const int size, const int round){
	int xPos = 9*padX + radCircle;
	const int yPos = 3.5*padY + 2*round*padY;
	for (int l = 0; l < correctPosition; l++){
	...
	for (int k = 0; k < correctCharacter - correctPosition; k++){
